...
export function ChatRoomScreen({route, navigation}: Props) {
  ...
  async function sendTip(amount: number) {
    ...
    if (error) { Alert.alert(error.message.includes('INSUFFICIENT') ? 'Not enough coins' : 'Tip failed', error.message.includes('INSUFFICIENT') ? 'Buy coins from Wallet with UPI and try again.' : error.message); return false; }
    await refreshViewer();
