...
const policies = {
  privacy: {title: 'Privacy policy', sections: [['Your information', 'We collect account, profile, device, chat, call, transaction and safety information needed to operate Kizo.'], ['Your controls', 'You can edit your profile, block or report users, change your password, and contact support about your account data.'], ['Retention', 'Direct chat messages expire after 24 hours. Profile media remains until removed. Safety and financial records may be retained as required.']]},
  terms: {title: 'Terms of service', sections: [['Adults only', 'Kizo is only for people aged 18 or older.'], ['Acceptable use', 'No impersonation, exploitation, threats, fraud, illegal content, or attempts to bypass safety and payment controls.'], ['Accounts and balances', 'Keep login details secure. Coins and beans are platform balances governed by the current rates and payout rules.']]},
...
export function PoliciesScreen({route}: Props) {
  ...
  return <View style={styles.root}><ScreenHeader back title={policy.title} eyebrow="Kizo policies" /><ScrollView contentContainerStyle={styles.content}>{policy.sections.map(([title, body]) => <View key={title} style={styles.section}><Text style={styles.title}>{title}</Text><Text style={styles.body}>{body}</Text></View>)}<WetButton title="Read full policy on wetdreams.vercel.app" variant="outline" onPress={() => void Linking.openURL(`${config.appUrl}/${route.params.page}`)} /></ScrollView></View>;
}
