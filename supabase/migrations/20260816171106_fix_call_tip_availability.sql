...
begin
  ...
  select * into v_receiver_user
  from public.users
  where id = v_receiver;

  if v_receiver_user.id is null or v_receiver_user.is_banned then
    raise exception 'USER_UNAVAILABLE' using errcode = 'P0001';
  end if;
  if v_receiver_user.role <> 'user' or v_receiver_user.is_guest then
    raise exception 'TIP_NOT_ALLOWED' using errcode = 'P0001';
  end if;
  if exists (
