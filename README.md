...
- The scheduled cleanup endpoint is `GET /api/maintenance/purge-expired-chats`. It accepts `Authorization: Bearer $CHAT_CLEANUP_SECRET` and is scheduled hourly through `vercel.json`; set the same value as `CRON_SECRET` in Vercel so hosted cron calls are authorized.
